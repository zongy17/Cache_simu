...
class CacheLine_handle {
public:
    CacheLine data;
    int way_id;
    CacheLine_handle(CacheLine line, int w): data(line), way_id(w) {}
    bool operator < (const CacheLine_handle & b) const {
        return data.time > b.data.time;//时间晚的排在前面
    }
};

void rearrange(std::vector<CacheLine> & cacheSet, std::vector<BitVector> & bitMap, int const A_ML, unsigned const major_mask) {
    int num_ways = cacheSet.size();
    assert(num_ways == bitMap.size());

    std::vector<CacheLine> new_cacheSet(num_ways);
    std::vector<BitVector> new_bitMap(num_ways);
    
    std::vector<CacheLine_handle> tmp_sort;
    std::unordered_map<int, int> map_oldway_2_newway;

    new_cacheSet[A_ML] = cacheSet[A_ML];
    map_oldway_2_newway[A_ML] = A_ML;

    for (unsigned i = 0; i < num_ways; i++)
        if (i != A_ML)
            tmp_sort.push_back(CacheLine_handle(cacheSet[i], i));
    std::sort(tmp_sort.begin(), tmp_sort.end());

    // 扫描第一遍，使得尽可能多的主位置得到满足
    for (std::vector<CacheLine_handle>::iterator it = tmp_sort.begin(); it != tmp_sort.end(); ){
        int my_major_loc = ML(it->data.tag, major_mask);
        if (new_cacheSet[my_major_loc].valid_bit != true) {// new_cacheSet这个位置还没被占，可以用作它的主位置
            new_cacheSet[my_major_loc] = it->data;
            map_oldway_2_newway[it->way_id] = my_major_loc;//记录从旧路到新路的变化关系
            tmp_sort.erase(it);// 从tmp_sort中删除
        } else {// 已经被占了，转向下一个
            it++;
        }
    }
    while (!tmp_sort.empty()) { // 此时tmp_sort剩下的是不能满足放到其主位置的
        int ptr = 0;
        while (new_cacheSet[ptr].valid_bit == true)
            ptr++;
        // 找到new_cacheSet中未被占的
        assert(new_cacheSet[ptr].time==0 && new_cacheSet[ptr].tag==0);
        new_cacheSet[ptr] = tmp_sort.begin()->data;
        map_oldway_2_newway[tmp_sort.begin()->way_id] = ptr;//记录从旧路到新路的变化关系
        tmp_sort.erase(tmp_sort.begin());
    }
    // 调整bitvector的指向关系
    assert(map_oldway_2_newway.size() == num_ways);
    for (std::unordered_map<int,int>::iterator it = map_oldway_2_newway.begin(); it != map_oldway_2_newway.end(); it++) {
        int old_way = it->first, new_way = it->second;
        BitVector old_bv = bitMap[old_way];
        BitVector & new_bv = new_bitMap[new_way];
        for (int old_w = 0; old_w < num_ways; old_w++) {
            if (old_bv.check_bit(old_w)) {
                int new_w = map_oldway_2_newway[old_w];
                assert(0<=new_w && new_w<num_ways);
                new_bv.active_bit(new_w);
            }
        }
    }

    int old_major_loc = 0, new_major_loc = 0;
    for (int w = 0; w < num_ways; w++){
        if (ML(cacheSet[w].tag, major_mask) == w){
            // printf(" old major loc: %d\n", w);
            old_major_loc++;
        }
        if (ML(new_cacheSet[w].tag, major_mask) == w){
            // printf(" new major loc: %d\n", w);
            new_major_loc++;
        }
            
    }
    if (old_major_loc != new_major_loc) {
        printf(" old: %d, new: %d\n", old_major_loc, new_major_loc);
        exit(1);
    }
    
    cacheSet = new_cacheSet;
    bitMap = new_bitMap;
}
            ...
            if (cacheSet[A_ML].tag == tag) {// 命中
                ...
                if (hit_line_id == -1) {// 都没有找到需要的，则从主存上调
                    ...
                    if (avail_line_id != -1) {// 有空行的
                        ...
                        // printf(" lines: %d, A_ML: %d\n", cnt_lines, A_ML);
                        // printf(" before:");
                        // for (int w = 0; w < num_ways; w++){
                        //     printf(" %lld", cacheSet[w].tag);
                        // }
                        // printf("\n");
                            
                        // Load完之后，除了A_ML路以外的其它路需要做一次数据重组
                        // 以使更多的主位置得到满足，如果有多个块的主位置相同，优先满足时间戳晚的
                        rearrange(cacheSet, bitvecSet, A_ML, major_mask);
